...
function doPullMedToPaper(tag, medicineName, qty, unitLabel, copies, data, exist, useTimes, usingTime, hasUsage, useQty, useUnit,specialInstructions,standard) {
    ...
    if (isNeddCreateNewMedDiv(data.useMode)) {
        var strs = createAllTypeDiv(data.useMode, "1", medicineName, qty, unitLabel, copies, useTimes, usingTime, tag, useQty, useUnit, data.groupId,data.specialInstructions,data.standard);
        ...
        var tmpMedDiv = createMedDiv(noode.size, medicineName, qty, unitLabel, copies, useTimes, usingTime, tag, useQty, useUnit, data.useMode,data.specialInstructions,data.standard);
        if (oldTypeName == data.useMode) {
            ...
            if (!exist) {
                ...
                if (groupName == groupNameArray[data.groupId]) {
                    ...
                    var groupNameDiv = '<div class="groupName" style="width:27%; float:left; font-size:16px; font-weight:bold;">' + groupNameArray[data.groupId] + fenHao + '</div>';
                    var pTop = '<div style="float:left; width:73%;">';
                    var pFloer = "</div>";
                ...
                if (groupName == groupNameArray[data.groupId]) {
                    ...
                    if (thisGroupNameNextBeiZhu1Div == "") {
                        ...
                        var pFloer = "</div>";
                        tmpMedDiv = createMedDiv(nowTypeHaveMedSize + 1, medicineName, qty, unitLabel, copies, useTimes, usingTime, tag, useQty, useUnit, data.useMode,data.specialInstructions,data.standard);
                        var endMendWithGroupDiv = ppTop + groupNameDiv + pTop + tmpMedDiv + pFloer + ppFloer;
 ...
 * @param fhf       饭后服  还是饭前
 */
function createMedDiv(num, medName, qty, danWei, size, pl, fhf, tagId, useQty, useUnit, oldTypeName,specialInstructions,standard) {
    //TODO 回滚
    //useUnit="";
    //useQty="";
    var fen = "#";
 ...
 * @param fhf       饭后服  还是饭前
 */
function createAllTypeDiv(typeName, num, medName, qty, danWei, size, pl, fhf, tagId, useQty, useUnit, groupId,specialInstructions,standard) {
    var fuSize = "";
    var fen = "#";
    var isJZ = qty + danWei + 'x' + size + fen;
    if (parseFloat(size) <= 0 || parseFloat(size) == 1) {
    ...
    groupText = '<div class="groupName" style="width:27%; float:left; font-size:16px; font-weight:bold;">' + groupNameArray[groupId] + fenHao + '</div>';
    var groupDivTop = '<div style="float:left; width:73%;">';
...
$(function () {
    for (var index = 1; index < oneMedInfo.length; index++) {
        ...
        doPullMedToPaper("medicineTag_" + data.medicineId,
            ...
            data.hasUsage,
            data.useQty,
            data.useUnit,data.specialInstructions,data.standard)
    }
})