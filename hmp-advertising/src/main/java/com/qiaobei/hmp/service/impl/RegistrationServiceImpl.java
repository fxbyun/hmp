...
import com.qiaobei.hmp.modules.entity.Registration_;
...
//        nowCallList.addAll(passWxList);
    ...
    public List<Registration> getByPatientAndDoctor(Patient patient, Doctor doctor) {
        return registrationDao.findAll((root, query, cb) -> {
            ...
            predicates.add(
                    cb.between(
                            root.get(Registration_.createOn),
                            DateUtils.getDayAfterHous(new Date(), 2, DateUtils.DATA_UTLIST_TYPE_DO.SUB),
                            DateUtils.getDayAfterHous(new Date(), 2, DateUtils.DATA_UTLIST_TYPE_DO.ADD)
                    )
            );
    ...
    public Long getRegistrationCountByDoctor(Doctor doctor) {
        return registrationDao.count((root, query, cb) -> cb.equal(root.get(Registration_.doctorId), doctor.getId()));
    }
    ...
    @Override
    public void setRegistrationCallStatusNot(Doctor doctor, String callName) {
        List<Long> doctorIds = Lists.newArrayList();
        doctorIds.add(doctor.getId());
        if (doctor.getDoctorType() == Doctor.Doctor_Type.Clinic_Boss) {
            doctorIds.addAll(doctorDao.findByprimaryDoctorId(doctor.getId()).stream().map(IdEntity::getId).collect(Collectors.toList()));
        }
        registrationDao.setRegistrationCallStatusNotCall(doctorIds, callName, Registration.CallStatus.DO_NOT_CALL);
    }
}
