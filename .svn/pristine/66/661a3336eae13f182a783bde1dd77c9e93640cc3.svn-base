...
<script>
    $(function () {
        $("#btnSearch").click(function () {
            ...
            var url = "${ctx}/adv/validityManage/entryRecord?goodsNo={0}".format(goodsNo);
            location.href = url;
        })
    })
    ...
    function fn_GoEntryRecord(page) {
        var goodsNo = $("#goodsNo").val();
        var url = "${ctx}/adv/validityManage/entryRecord?goodsNo={0}&page={1}".format(goodsNo, page);
        location.href = url;
    }


    function toAddIaiDetail() {
        var url = "${ctx}/adv/validityManage/isHistory";
            ...
            if (data.success) {
                ...
                layer.confirm("你有一张历史入库单，是否继续编辑？", {
                    ...
                    btn1: function () {
                        ...
                        var addUrl = "${ctx}/adv/validityManage/saveValList?addUUID={0}".format(addUUID);
                        location.href = addUrl;
                    },
                    btn2: function () {

                        var editUrl = "${ctx}/adv/validityManage/saveValList?addUUID={0}".format(iaiUUID);
                        location.href = editUrl;
                    }
                });
            } else {
                var addUUID = Math.uuidFast();
                var url = "${ctx}/adv/validityManage/saveValList?addUUID={0}".format(addUUID);
                location.href = url;
            }
