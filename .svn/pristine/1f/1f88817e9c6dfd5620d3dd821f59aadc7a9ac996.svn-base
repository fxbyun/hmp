...
import com.qiaobei.hmp.schedule.AutoSendMessagePreocess;
...
/**
 * Created by teemoer@cntv.cn on 2016/7/7 0007.
 */
@RunWith(SpringJUnit4ClassRunner.class)
@ContextConfiguration(locations = "classpath:applicationContext-junit.xml")
@TransactionConfiguration(transactionManager = "transactionManager")
//@ActiveProfiles(Profiles.UNIT_TEST)
public class AutoSendMessageJunit {
    private static SimpleDateFormat sdf = new SimpleDateFormat("MM月dd日"); //设置时间格式
    ...
    private static String pullMsg(Emr emr, Doctor doctor) {
        StringBuffer stringBuffer = new StringBuffer(emr.getPatientName() + "，感谢您于");
        stringBuffer.append(sdf.format(emr.getCreateOn()));
        stringBuffer.append("选择");
        stringBuffer.append(doctor.getOutpatientService());
        stringBuffer.append("就诊，");
        stringBuffer.append("不知您是否已康复，若有需要，可到门诊复查或咨询");
        stringBuffer.append(doctor.getMobile() + "。");
    ...
    public void testRedis() {
        ...
        if (wxInfo == null) {
            System.out.println("wxToken 是 空的 进入生产方法");
            wxInfo = new WxInfo();
            wxInfo.setACCESS_TOKEN_PHAR("ACCESS_TOKEN_PHAR");
            wxInfo.setACCESS_TOKEN_OP("ACCESS_TOKEN_OP");
            wxCache.addOrUpdate("wxToken", wxInfo, 1);
        }

        WxInfo wxInfo_New = wxCache.load("wxToken");

        System.out.println(wxInfo_New);
    ...
    public void sendMsgTest() {
        try {
            SMSUtil.sendSMS(SMSUtil.CUSTOMIZE_ALONE_TEMLATE, "18813925358", "黄羊 感谢您在我们诊所就诊,请问您现在身体是否安康?");
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
        ...
        for (Emr emr : emrList) {
            ...
            if (isNeddAutoSend) {
                ...
                if (emr.getSendMsgInfo() != null && !"".equals(emr.getSendMsgInfo())) {
                    ...
                    int sendOneSize = (int) Math.ceil(Double.valueOf(sendStr.length()) /
