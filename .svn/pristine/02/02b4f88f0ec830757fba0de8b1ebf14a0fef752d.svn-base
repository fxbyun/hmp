...
public interface MedicineDao extends JpaRepository<Medicine, Long>, JpaSpecificationExecutor<Medicine> {
    ...
    @Query("select m from Medicine m " +
            "where m.type=:type GROUP BY m.name")
    List<Medicine> findAllMedByType(@Param("type") Medicine.Type type);

    @Query("select m from Medicine m " +
            " GROUP BY m.name")
    List<Medicine> findAllMedList();
    ...
    @Query("select m from Medicine m " +
            "where   (m.name like :name or m.helpCode " +
            "like:name)  GROUP BY m.name")
    List<Medicine> findAllMedListAndNameLike(@Param("name") String name);
    ...
    @Query("select m from Medicine m left join m.doctorMedicineList dm " +
            "where (dm.doctor <>:doctor or dm.doctor is null) and m.type=:type group by m.name")
