...
function fn_RemoveElement(ele, status) {
    if (status != "Normal" && status != "CHARGE" && status != undefined && status != null && status != "undefined") {
    ...
    if (status == undefined) {
        ...
        if (indexWz >= -1) {
            ...
            if (ele.parentElement.id == 'divDiagnosis') {
                //删除珍后建议
                $("#divSuggest").children(".diagnosis_" + ele.firstElementChild.value).remove();
                //删除初步诊断标签
...
function removeTeSun(ele) {
    try {
        if ($("#divSuggest").find("div").size() == 1) {
            ...
            ele.remove();
            divSuggestHtml = $('#divSuggest').html();
        }
    } catch (e) {
        ...
        if (res && res.success) {
            $.each(res.data, function (i, n) {
                if ($.inArray(n.content, arr) == -1) {
                    var ele = '<div class="{1}" style="font-size: 16px">{0}'.format(n.content, "diagnosis_" + tagId) +
                        '<a href="javascript:" onclick="removeTeSun($(this).parent());/*fn_RemoveElement($(this).parent())*/"><i class="fa fa-times"></i></a>' +
...
/*开始坐诊药品选择方法（选择药品弹窗）*/
function fn_ShowSelectMedicine(medicineId, medType, handleType) {
    //默认是add 添加要的类型
    handleType = 'Medicine_Add';
    //弹出框的宽高
    var height = "";
    var width = "";
    ...
    if ($.inArray(tmp, arr) != -1) {
        ...
        layer.open({
            ...
            area: [height, width],
            scrollbar: false,
            content: '/fragment/medicine/select/{0}?type={1}&isShowWindow=true&selectMedUseModType={2}&lastSelectXwOrFenBao={3}&handleType={4}'.format(medicineId, types, selectMedUseModType, lastSelectXwOrFenBao, handleType)
        });
    }
}
...
function fn_ShowEditMedicine(medicineId, types, tagMedId, medType, status) {
    ...
    layer.open({
        ...
        + "&handleType=" + decodeURI("Medicine_Edit")
    });
...
function deleteItemTr(id) {
    layer.confirm("是否删除此项目！(可以在创建检查/检验项目中重新添加)", function () {
...
function pullItToPager(obj) {
    ...
    if ($(obj).val() == "") {
        ...
        $("#talkSmtz span").each(function () {
            if (needDeleteText == '其他' && $(this).prop("id") == 'ot_text') {
                $(this).remove();
            } else {
        ...
        //如果是‘其他’
        if (needDeleteText == '其他') {
            var enety = '<span class="tag" id="ot_text">' + str + '</span>';
        } else {
