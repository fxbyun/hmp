...
public class MessageController {
    ...
    public String msAddPeople(@RequestParam(value = "page", required = false, defaultValue = "0") int page,
                              ...
                              Model model,
                              HttpSession session) {
        Map map = (Map) session.getAttribute("chackPatient");
        if (null == map) {
        ...
        Page<Emr> patientListEntities = emrService.findCountByPatient(
    ...
    @RequestMapping("/msPage/seachDiag")
    @ResponseBody
    public List seachDiag(@RequestParam("key") String key) {
        List<DiagnosisTag> diagnosisList = Lists.newArrayList();
        tagService.getByNameOrHelpCode(key).forEach(s -> diagnosisList.add(new DiagnosisTag(s)));
    ...
    public List<Patient> getPatientInfo(HttpSession session, @RequestParam(value = "type", required = false) String
        ...
        Map map = (Map) session.getAttribute("chackPatient");
        session.setAttribute("chackPatient", null);
        List<Patient> patienList = Lists.newArrayList();
        if (null != map && map.size() > 0) {
            for (Object o : map.keySet()) {
                Patient patient = patientService.getPatientById((Long) o);
    ...
    public Result addSendPatienInfo(@RequestParam("patientIdList") List<Long> patientIdList,
                                    ...
                                    @RequestParam(required = false) @DateTimeFormat(pattern = Constants.PATTERN_DATE)
                                            Date txtEndDate,
                                    @RequestParam(value = "msgContext") String msgContext,
                                    @RequestParam(value = "title") String title,
                                    HttpSession session) {
        if (patientIdList != null && patientIdList.size() > 0) {
            //System.out.println(msgContext.length());
            int sendOneSize = (int) Math.ceil((double) msgContext.length() /
    ...
    public Map clearSessionMsgInfo(HttpSession session) {
        ...
        return msgSendHistoryService.getDoctorMsgInfo(SecuritySupport.getDoctor());
    }
    ...
    public Result sendMsg(HttpSession session) {
        ...
        List patientIdList = (List) session.getAttribute("patientIdListMsg");
        List mobListMsg = (List) session.getAttribute("mobListMsg");
        ...
        msgSendHistory.setMsgSendHistoryDetail(Lists.newArrayList());
        ...
        for (int i = 0; i < patientIdList.size(); i++) {
            ...
            msgSendHistoryDetail.setPatient(new Patient((Long) patientIdList.get(i)));
            msgSendHistoryDetail.setMsgSendHistory(msgSendHistory);
            try {
                //TODO 暂时屏蔽
                SMSUtil.sendSMS(SMSUtil.CUSTOMIZE_ALONE_TEMLATE, (String) mobListMsg.get(i), msgContextMsg);
    ...
    public String loadSmgSendHosDetil(@RequestParam(value = "id") Long id,
