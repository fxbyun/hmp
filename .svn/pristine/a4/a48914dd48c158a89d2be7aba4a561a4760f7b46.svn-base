...
<div class="scroll_demo" onmousewheel="showKey()">
    <c:forEach items="${list.content}" var="one">
        <li>
            <a href="javascript:" onclick="saveMedicine(${one.id},${iai.id})">
...
<script>
    ...
    function scrollPageTag(flag,id) {
        var medName = $("#medNameInput").val();
        var iaiLossId = id;
        var page = ${list.number}
        var totalNum = ${list.totalPages}
        if(flag=="left"){
            if(page-1<0){
                layer.msg("这是已经是第一页了！")
                return false
            }
            loadMedInfoList(medName,page-1,iaiLossId);
        }else {
            if(page+1>totalNum){
