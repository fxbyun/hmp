                    ...
                    <div class="pull-left" style="width: 50%; padding-right: 5%;">
                        <div class="form-group text-right">
                            <span>报损时间：</span><input type="text" name="createDate" readonly
                                                     value="<c:if test="${empty iaiLoss.createDate}"><fmt:formatDate value="${now}" pattern="yyyy/MM/dd"/></c:if><c:if test="${not empty iaiLoss.createDate}"><fmt:formatDate value="${iaiLoss.createDate}" pattern="yyyy/MM/dd"/></c:if>"
...
<script>
    $(function () {
        ...
        $("#btnSaveLoss").click(function () {
            $("#saveLoss").ajaxSubmit({
                url: "${ctx}/adv/IaiLossManage/saveLoss",
    ...
    function selectExpireMed(name, page, iaiLossId) {
        ...
        if (iaiLossId == undefined) {
            url = "${ctx}/adv/IaiLossManage/bombBox/expireMedList?name={0}&page={1}".format(name, page);
        } else {
            url = "${ctx}/adv/IaiLossManage/bombBox/expireMedList?name={0}&page={1}&iaiLossId={2}".format(name, page, iaiLossId);
        }
    ...
    function deleteLossDetail(even, lossDetailId) {
        ...
        layer.confirm("你确定要删除该损耗药品？", function () {
            $.postJSON("${ctx}/adv/IaiLossManage/bombBox/delLossItem", {"lossDetailId": lossDetailId}, function (data) {
                if (data.success == true) {
                    layer.msg("损耗药品已成功删除！")
                } else {
                    layer.msg("损耗药品删除失败！");
