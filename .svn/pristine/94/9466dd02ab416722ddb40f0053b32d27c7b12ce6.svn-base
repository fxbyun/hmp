...
function loadMedInfoList(name, page, iaiIntoId) {
    ...
    if (iaiIntoId == undefined) {
        url = "/adv/validityManage/bombBox/getMedInfo?name={0}&page={1}".format(name, page);
    } else {
        url = "/adv/validityManage/bombBox/getMedInfo?name={0}&page={1}&iaiIntoId={2}".format(name, page, iaiIntoId);
    }
...
function deleteValListTr(even, detailId) {
    $.postJSON("/adv/validityManage/delDetail", {"detailId": detailId}, function (data) {
...
function deleteIaiInto(even, iaiIntoId) {
    ...
    layer.confirm("你确定要删除该入库订单？", function () {
        $.postJSON("/adv/validityManage/delIaiInto", {"iaiIntoId": iaiIntoId}, function (data) {
            if (data.success == true) {
                layer.msg("入库单已成功删除！")
            } else {
                layer.msg("入库单删除失败！");
...
function deleteIaiLoss(even, lossId) {
    ...
    layer.confirm("你确定要删除该损耗单？", function () {
        $.postJSON("/adv/validityManage/delIaiLoss", {"lossId": lossId}, function (data) {
            if (data.success == true) {
                layer.msg("损耗单已成功删除！")
            } else {
                layer.msg("损耗单删除失败！");
...
function saveMedicine(medId, iaiIntoId, iaiDetailId, flag) {
    ...
    if (iaiDetailId == undefined || iaiDetailId == "") {
        url = "/adv/validityManage/bombBox/saveMedicine?medId={0}&iaiIntoId={1}&fromHtml={2}".format(medId, iaiIntoId, flag);
    } else {
        url = '/adv/validityManage/bombBox/saveMedicine?medId={0}&iaiIntoId={1}&iaiDetailId={2}&fromHtml={3}'.format(medId, iaiIntoId, iaiDetailId, flag);
    }
    layer.open({
        type: 2,
        maxmin: false,
...
function showOredrDetail(even) {
    window.location.href = "/adv/validityManage/replenishment";
}
...
function orderDetails() {
    layer.open({
        ...
        area: ['400px', '360px'],
        scrollbar: false,
        content: '/adv/validityManage/bombBox/showReplenish'
    });
}
...
function supplierManage(obj) {
    ...
    var url = "/adv/validityManage/bombBox/manageSupplier";
    layer.open({
        type: 2,
        maxmin: false,
        title: '供应商管理',
        area: ['580px', '420px'],
        scrollbar: false,
        content: '/adv/validityManage/bombBox/manageSupplier'
    });
}
...
function fn_EditSupplier(supplierId) {
    layer.open({
        ...
        area: ['400px', '180px'],
        scrollbar: false,
        content: '/adv/validityManage/bombBox/editSupplier?supplierId=' + supplierId,
        end: supplierReload
    });
}
...
function fn_AddSupplier() {
    layer.open({
        ...
        area: ['400px', '180px'],
        scrollbar: false,
        content: '/adv/validityManage/bombBox/addSupplier',
        end: supplierReload
    });
}
...
function delSupplier(ele, supplierId) {
    layer.confirm("你确定要删除该供应商吗？", function () {
        var url = "/adv/validityManage/bombBox/delSupplier";
...
function saveSupplier(supplierId) {
    ...
    var url = "/adv/validityManage/bombBox/saveSupplier";
...
function editMedicine() {
    layer.open({
        ...
        area: ['700px', '450px'],
        scrollbar: false,
        content: '/adv/validityManage/bombBox/editMedicine'
    });
}

function fn_LoadIaiIntoDetailPage(iaiIntoId, page) {
    $("#iaiDetail").load("/adv/fragment/validityManage/showIaiDetails?iaiIntoId={0}&page={1}".format(iaiIntoId, page));
}
