                        ...
                        <div class="row-fluid">
                            <div style="padding: 15px;width: 700px;margin: 0 auto;">
                                <form:form id="advertForm" modelAttribute="adverting" action="${ctx}/adverting/add"
                                    ...
                                    <div id="textArea">
                                        <label><b>内容：</b></label>
                                        <textarea id="tinymce_basic" name="content"
                                                  style="height:100px;width: 100%">${adverting.content}</textarea>
                                    </div>
                                    <div id="picArea">
                                        <label><b>图片：</b></label>
                                    ...
                                    <form:input path="indate" cssClass="form-control"
                                                cssStyle="width:80px" /> 天&nbsp;&nbsp;为空或设置0，则表示永不过期。
                                    <br />

                                    <label><b>排序：</b></label>
                                    <form:input path="orderNo" cssClass="form-control"
                                                cssStyle="width:80px" /> 1则表示放在第一位,如设置了2个1那么放到之前已存在的照片/视频后面。
                                    <br />
                                    <br />
                                    <button type="button" class="btn btn-info" id="btn_save">保存</button>
                                    &nbsp;&nbsp;
                                    <a href="${ctx}/adverting/list" class="btn btn-info">返回列表</a>&nbsp;&nbsp;
                                    <c:if test="${not empty adverting.id}">
...
//        });
        ...
        $("#del").click(function () {
            layer.confirm('确定删除？', {
                ...
                var id = $("#adId").val();
                location.href = "${ctx}/adverting/del/" + id;
            });
        });
    });
</script>