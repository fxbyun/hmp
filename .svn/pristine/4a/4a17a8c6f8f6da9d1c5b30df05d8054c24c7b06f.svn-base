...
<head>
    ...
    <script>
        ...
        function toLink(emrId) {
            window.location.href = ("${ctx}/oldPatient/oldPtDetail?emrId={0}".format(emrId));
        }
        $(function () {
        ...
        function goToThisPage() {
            window.location.href = "${ctx}/oldPatient/getAllByDoctor";
        }
    </script>
</head>
<body>
<div class="container electronic tabbable">
    <form id="patientManagerForm" action="${ctx}/oldPatient/oldPatientManageEmrList" method="post">
        <input id="hidPage" type="hidden" name="page" />
        <input id="hidPatientId" type="hidden" name="patientId" />


        <div class="row oldPatientList">
            <div class="col-md-12">
                            ...
                            <div class="form-group">
                                <ul class="nav nav-tabs" role="tablist">
                                    <li role="presentation"><a href="#manage" class="btn btn-default"
                                                               ...
                                                               data-toggle="tab" onclick="goToThisPage()">我的患者</a>
                                    </li>
                        ...
                        <div class="intro-sign form-group">
                            <table class="case-table table-hover">
                                ...
                                <tr onclick="toLink(${emr.id})">
                                    <td><fmt:formatDate value="${emr.createTime }" pattern="yyyy/MM/dd hh:mm:ss" /></td>
                                    <td>${emr.oldPatient.name}</td>
                                    <td>${emr.oldPatient.age}</td>
                                    <td><c:if test="${emr.oldPatient.gender eq 'Male'}">男</c:if><c:if
                                            test="${emr.oldPatient.gender eq 'Female'}">女</c:if></td>
                                    <td>${emr.zhenDuan}</td>
                                </tr>
                                </c:forEach>
                            </table>
