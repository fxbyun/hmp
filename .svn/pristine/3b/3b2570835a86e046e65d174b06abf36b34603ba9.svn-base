...
public class MedicinePrivateServiceImpl implements MedicinePrivateService {
    ...
    @Resource
    MedicineCountService medicineCountService;
    ...
    public List<MedicinePrivate> isDoctorHasThisTherapy(Doctor doctor, Medicine medicine) {
        return medicinePrivateDao.findByDoctorAndMedicine(doctor, medicine).stream().filter(
    ...
    public Page<MedicinePrivate> getByTypeAndCategoryAndDoctor(Pageable page, Doctor doctor, Medicine.Type type, String category) {
        Page<MedicinePrivate> medicinePrivatePage = medicinePrivateDao.findByTypeAndCategoryAndDoctor(page, doctor, type, category);
    ...
    public String getStandardMoreDoctorUse(Medicine medicine) {
        Page<MedicineContainer> mcPage = medicinePrivateDao.findstandardMoreDoctorUse(medicine, new PageRequest(0, 1));
    ...
    public Double getRateMoreDoctorUse(Medicine medicine) {
        Page<MedicineContainer> mcPage = medicinePrivateDao.findRateMoreDoctorUse(medicine, new PageRequest(0, 1));
        if (CollectionUtils.isNotEmpty(mcPage.getContent())) {
    ...
    public String getQtyMoreDoctorUse(Medicine medicine) {
        Page<MedicineContainer> mcPage = medicinePrivateDao.findQtyMoreDoctorUse(medicine, new PageRequest(0, 1));
        if (CollectionUtils.isNotEmpty(mcPage.getContent())) {
            return mcPage.getContent().get(0).getStrMoreUser();
        } else {
            return null;
    ...
    public Boolean getUsageFlagMoreDoctorUse(Medicine medicine) {
        Page<MedicineContainer> mcPage = medicinePrivateDao.findUsageFlagMoreDoctorUse(medicine, new PageRequest(0, 1));
        if (CollectionUtils.isNotEmpty(mcPage.getContent())) {
    ...
    public String getUseModelMoreDoctorUse(Medicine medicine) {
        Page<MedicineContainer> mcPage = medicinePrivateDao.findUseModelMoreDoctorUse(medicine, new PageRequest(0, 1));
        if (CollectionUtils.isNotEmpty(mcPage.getContent())) {
            return mcPage.getContent().get(0).getStrMoreUser();
        } else {
            return null;
    ...
    public String getCategoryMoreDoctorUse(Medicine medicine) {
        Page<MedicineContainer> mcPage = medicinePrivateDao.findCategoreMoreDoctorUse(medicine, new PageRequest(0, 1));
        if (CollectionUtils.isNotEmpty(mcPage.getContent())) {
            return mcPage.getContent().get(0).getStrMoreUser();
        } else {
            return null;
    ...
    public List<MedicinePrivate> findByDoctorAndMedicineName(Doctor doctor, String medName) {
        return medicinePrivateDao.findAll((root, query, cb) -> cb.and(
                cb.equal(
