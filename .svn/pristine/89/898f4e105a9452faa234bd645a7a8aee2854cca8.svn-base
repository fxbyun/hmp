...
public class PublicController extends ConstantsController {
    ...
    public String printChargesTable(@RequestParam("emrId") Long emrId,
        ...
        Double fujiaPrices = emrExtCostList.stream().filter(emrExtCost -> emrExtCost.getPrice() != null).mapToDouble(EmrExtCost::getPrice).sum();
        model.addAttribute("fujiaPrices", fujiaPrices);
    ...
    public String printPhyTable(Model model, @RequestParam(value = "emrId") Long emrId) {
        ...
        //获取单据价格
        Double therapyPrice = therapyList.stream()
                .filter(emrMedicine -> emrMedicine.getPrice() != null)
                .mapToDouble(EmrMedicine::getPrice)
                .sum();
        ...
        model.addAttribute("therapyPrice", therapyPrice);
        model.addAttribute("emr", emr);
