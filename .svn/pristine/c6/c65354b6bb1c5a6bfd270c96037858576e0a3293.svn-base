...
<div class="main mCenter" style="min-height: 198mm; position: relative;" id="printDiv">
    <table style="padding: 0 20px;">
        ...
        <tr class="main-sTitle" style="line-height: 1.5em;">
            <td style="width: 30%">
                <span>患者姓名:</span>
                <input type="text" class="" style="width: 4.2em" value="${emr.patientName}" readonly/>
            </td>
            <td style="width: 15%">
                <span>性别:</span>
                <input type="text" style="width: 1.8em" class=" " value="${genderMap[emr.patient.gender]}"
                       readonly/>
            </td>
        ...
        <tr class="main-sTitle" style="line-height: 1.5em;">
            <td colspan="4">
                <span>患者主诉:</span>
                <input type="text" class=" " style="width: 82%" value="
                                ...
                                <c:if test="${doctor.needAlonePrinTypeStrings.contains(entry.key)}">
                                    ...
                                    <c:forEach varStatus="status2" var="em2" items="${entry.value}">
                                        <tr style="line-height: 1.8em;">
                                            <td colspan="2">
                                                <strong>${entry.key}
                                                    <c:if test="${'煎服'.equals(entry.key)}">(${emr.chineseQty}副)</c:if></strong>
                                                </strong>
                                                <strong>
                                                    <c:choose>
                                                        <c:when test="${(xwString.indexOf(entry.key)>=0) && not empty em2.key && em2.key>0}">
