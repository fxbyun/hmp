...
<div class="container chargeManage">
    <div class="navigation">
        <ul class="clearfix">
            ...
            <li class="active"><a href="${ctx}/adv/chargeManage?status=CHARGE" class="btn btn-default">已收费列表</a></li>
    ...
    <form action="${ctx}/adv/chargeManage" method="post" id="formSubmit">
        ...
        <input id="status" type="hidden" name="status" value="CHARGE"/>
        <div class="adv-container alreadyList">
            ...
            <table class="adv-table" width="100%">
                ...
                <c:forEach items="${emrListPage.content}" varStatus="status"
                           var="one"><%--@elvariable id="phar" type="com.qiaobei.hmp.modules.entity.Pharmacist"--%>
                    <tr
                            <c:if test="${one.status=='Normal'}">hmp-status-call</c:if>
                    >
                        <td>
                        ...
                        <td> ${not empty nurseMap[one]?nurseMap[one].name:one.doctor.name}</td>
                        <td><span class="text-num" title="状态">
                            <c:if test="${one.status!='Normal'}">${one.status.getName()}</c:if>
                            <c:if test="${one.status=='Normal'}">有新药品</c:if>
                        </span></td>
                        <td>
                            <button class="btn btn-default" type="button"
                                    onclick="alreadyInfo(${one.id},'${one.status}','${phar.isChiefNurse}')">退费
                            </button>
