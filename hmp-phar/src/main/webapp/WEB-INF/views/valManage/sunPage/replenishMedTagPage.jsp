...
<div class="scroll_demo" onmousewheel="showKey()">
    <c:forEach items="${list.content}" var="one">
        <li>
            <a href="javascript:" onclick="replenishMed(${one.id},${iai.id})">
...
<script>
    ...
    function replenishMed(medPrivateId, iaiIntoId) {
        var url = "${ctx}/adv/validityManage/bombBox/saveMedicine?medId={0}&iaiIntoId={1}&fromHtml={2}".format(medPrivateId, iaiIntoId, 'replenishment');
        layer.open({
            type: 2,
            maxmin: false,
            title: '编辑药品',
