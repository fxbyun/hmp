...
import com.qiaobei.hmp.modules.support.RandomValidateCode;
...
public class AnonController extends BaseController {
    ...
    private static EnumMap<Gender, String> genderMap = new EnumMap<>(Gender.class);
    ...
    @Autowired
    public AnonController(DoctorService doctorService, NationService nationService, EmrService emrService) {
     ...
     * @return
     */
    @RequestMapping(value = "isRandCode")
    @ResponseBody
    public Result isRandCode(@RequestParam(value = "identifyCode") String identifyCode, HttpServletRequest request) {
        if (null != identifyCode) {
            String randCode = request.getSession().getAttribute(RandomValidateCode.RANDOMCODEKEY).toString();
