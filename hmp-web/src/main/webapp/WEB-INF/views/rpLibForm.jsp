    ...
    <script type="text/javascript">
        ...
        function fn_SelectMedicine(data) {
            ...
            var ele = '<span class="tag" id="{0}">{1}&nbsp;{2}{3}x{4}份'.format(tag, data.medicineName, data.qty, data.unitLabel, data.copies) +
                    ...
                    '<a href="javascript:" onclick="fn_ShowEditMedicine({0})"><i class="fa fa-cog"></i></a> '.format(data.medicineId) +
...
<div style="margin: 15px; background-color: #fff; border-radius: 10px; padding: 50px;">
    <form:form action="/rp/saveByLib" cssClass="form-horizontal" method="post" modelAttribute="rp">
        ...
        <div class="form-group">
            ...
            <div class="col-xs-10 col-sm-10">
                <form:bsradiobuttons path="medicineType" items="${medicineTypes}" readonly="true" labelCssClass="radio-inline"/>
            </div>
        </div>
        <div class="form-group">
                ...
                <div id="divItems" class="label-box">
                    <c:forEach var="item" items="${rp.prescriptionItemList}">
                        <span class="tag" id="rpMedicineTag_${item.medicineId}">${item.medicineName}&nbsp;${item.qty}${medicineUnits[item.unit]}x${item.copies}份
                            ...
                            <a href="javascript:" onclick="fn_ShowEditMedicine(${item.medicineId})"><i class="fa fa-cog"></i></a>
                            <a href="javascript:" onclick="fn_RemoveElement($(this).parent())"><i class="fa fa-times"></i></a>
                        </span>
                    </c:forEach>
                </div>
        ...
        <div class="form-group">
            <div class="col-xs-offset-2 col-sm-offset-2 col-xs-4 col-sm-4">
                <button type="submit" class="btn btn-success"><i class="fa fa-save"></i> 保存至我的药方</button>
                <button id="btnClose" type="button" class="btn btn-default"><i class="fa fa-times"></i> 关闭</button>
            </div>
            <div class="col-xs-6 col-sm-6"><h5 class="text-danger">${msg}</h5></div>
        </div>
