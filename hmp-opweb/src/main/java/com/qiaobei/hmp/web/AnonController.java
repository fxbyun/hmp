...
public class AnonController extends BaseController{
     ...
     * 发送验证码
     */
    @RequestMapping("/anon/sendAuthCode")
    @ResponseBody
        ...
        try {
            ...
            if (StringUtils.isNotEmpty(callback)){
                ...
                if(StringUtils.equals(SMSUtil.SUCCESS_CODE, json.getString("respCode"))){
                    //获取session存在的时间
                    System.out.println("session的闯将时间:" + request.getSession().getCreationTime());
                    if (null != request.getSession().getAttribute("sessionCreateTime")) {
                        Long firstTime = Long.parseLong(request.getSession().getAttribute("sessionCreateTime").toString());
                        long diffTime = (new Date().getTime()) - firstTime.longValue();
                        if (diffTime / 1000 <= 3) {
                            System.out.println("验证在60秒内被申请两次!");
                        } else {
                            request.getSession().setAttribute("sessionCreateTime", (new Date()).getTime());
    ...
    @RequestMapping(value = "/anon/save", method = RequestMethod.POST)
