        ...
        <div class="col-md-10 col-sm-10 my-infro-right">
                ...
                <c:forEach items="${page.content}" var="item">
                    <div class="action-now-content">
                        <div class="action-item">
                            <h3 onclick="fn_ChangeStatus(${item.id},'${item.status}')" href="#content${item.id}" role="button" data-toggle="collapse" aria-expanded="true" aria-controls="content${item.id}">
                                ...
                                ${item.notice.subject}
                                <span class="pull-right"><fmt:formatDate value="${item.notice.createOn}" pattern="yyyy/MM/dd HH:mm"/></span>
                            </h3>
                            <div id="content${item.id}" class="panel-collapse collapse " role="tabpanel" aria-labelledby="headingone">
            ...
            <div class="fenye">
                <div class="form-group pull-right text-right">
                    第 <input type="text" class="form-control" id="currentPage" value="${page.number + 1}">页/共<span>${page.totalPages}</span>页
                </div>
            </div>
            <div class="fenye-buttom text-center">
                <c:choose>
                    <c:when test="${page.number > 0}">
                        <button type="button" onclick="fn_LoadPage(${page.number - 1})" class="btn btn-info"><i class="fa fa-chevron-left"></i> 上一页</button>
                    </c:when>
                    <c:when test="${page.number + 1 < page.totalPages}">
                        <button type="button" onclick="fn_LoadPage(${page.number + 1})" class="btn btn-info"><i class="fa fa-chevron-right"></i> 下一页</button>
                    </c:when>
                </c:choose>
            </div>
        </div>
