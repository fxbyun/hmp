    ...
    <script>
        $(function () {
            ...
            $("#btnSure").click(function () {
                ...
                $.postJSON(ctx + "/retail/getPatientInfo", {
                    "udid": $("#udid").val(),
                    "mobile": $("#mobile").val(),
                }, function (result) {
                    if (result.success) {
                        ...
                        parent.window.location.href = "${ctx}/retail/RetailBilling?patientId={0}".format(result.data.id);
            ...
            $('#mobile').typeahead({
                ...
                templates: {
                    ...
                    'suggestion': function (o) {
                        ...
                        var div = "<div onclick='commit({3})'>{0} - <strong>{1}</strong> – {2}</div>";
                        return div.format("请选择", o.uid, name, o.id);
                    }
                }
            });
        });

        function commit(patientId) {

            parent.window.location.href = "${ctx}/retail/RetailBilling?patientId={0}".format(patientId);
            parent.layer.close(index);
        }
    </script>
</head>
...
<div style="margin: 2em 2em; line-height: 2.8em;" class="text-center">
    ...
    <div class="form-group clearfix" style="line-height: 2em;">
        <span style="float:left; line-height: 1em; padding-right: 37px;">手动添加</span>
        <input type="text" id="mobile" class="form-control"
               style="float:left; width: 100%;"
               placeholder="请输入手机号">
    </div>
    <div class="form-group text-center" style="margin-top: 2em;">
        <a type="button" class="btn btn-success" id="btnSure">确认</a>
